...
class SlidingWindowMaximum:
    def maxSlidingWindow(self, nums, k):
        ...
        for i in range(len(nums)):
            ...
            # Remove indices where the corresponding values are less than the current element
            while deque_window and nums[deque_window[-1]] < nums[i]:
                deque_window.pop()
            
            deque_window.append(i)
            
            # If we have processed at least k elements, add the max of the window to the result
            if i >= k - 1:
