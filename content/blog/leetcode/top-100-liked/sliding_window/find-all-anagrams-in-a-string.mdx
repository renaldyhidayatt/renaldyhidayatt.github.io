...
class FindAllAnagrams:
    def findAnagrams(self, s: str, p: str):
        ...
        # Fill frequency arrays for p and the first window of s
        for i in range(len(p)):
            freqP[ord(p[i]) - ord('a')] += 1
            freqS[ord(s[i]) - ord('a')] += 1
        
        # If initial window matches, add the index
        if freqP == freqS:
            result.append(0)
        ...
        for i in range(len(p), len(s)):
            ...
            # If the frequency arrays match, add the index to the result
            if freqP == freqS:
...
if __name__ == "__main__":
    solver = FindAllAnagrams()
    print(solver.findAnagrams("cbaebabacd", "abc"))  # Output: [0, 6]
```
