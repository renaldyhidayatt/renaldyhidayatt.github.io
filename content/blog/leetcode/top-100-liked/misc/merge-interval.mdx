...
class MergeIntervals:
    def merge(self, intervals):
        ...
        for interval in intervals:
            # If merged is empty or the current interval does not overlap with the last one, add it to the merged list
            if not merged or merged[-1][1] < interval[0]:
                merged.append(interval)
            else:
                # Otherwise, merge the intervals by updating the end of the last interval
...
if __name__ == "__main__":
    mi = MergeIntervals()
    intervals = [[1,3],[2,6],[8,10],[15,18]]
    print(mi.merge(intervals))  # Output: [[1, 6], [8, 10], [15, 18]]
```
