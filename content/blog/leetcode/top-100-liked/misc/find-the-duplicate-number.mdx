...
if __name__ == "__main__":
    fd = FindDuplicate()
    nums = [1, 3, 4, 2, 2]
    print(fd.findDuplicate(nums))  # Output: 2
```
...
The **Find the Duplicate Number** problem can be solved efficiently using **Floyd’s Tortoise and Hare Algorithm**, achieving `O(n)` time complexity with `O(1)` space. 🚀
