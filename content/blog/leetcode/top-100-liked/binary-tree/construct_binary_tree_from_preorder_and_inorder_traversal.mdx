...
class Solution:
    def buildTree(self, preorder: List[int], inorder: List[int]) -> Optional[TreeNode]:
        ...
        def helper(preIndex: int, inStart: int, inEnd: int) -> Optional[TreeNode]:
            if inStart > inEnd:
                return None
            ...
            left_subtree_size = index - inStart
            root.left = helper(preIndex + 1, inStart, index - 1)
            root.right = helper(preIndex + left_subtree_size + 1, index + 1, inEnd)
