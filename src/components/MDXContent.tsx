...
export const sharedComponents = {
    Latex: ({ children }: { children: string }) => (
        <div className="overflow-x-auto mb-8">
            <div className="whitespace-pre-wrap break-words text-base text-foreground bg-accent/30 p-6 rounded-xl border border-border">
    ...
    thead: ({ children }: { children: React.ReactNode }) => (
        <thead className="bg-accent/50 backdrop-blur-sm">
            {children}
        </thead>
    ),
    ...
    th: ({ children }: { children: React.ReactNode }) => (
        <th className="px-6 py-4 text-left font-semibold text-primary border-b-2 border-border">
            {children}
        </th>
    ),
    ...
    pre: ({ children }: { children: React.ReactNode }) => {
        ...
        return (
            <pre className="bg-card p-6 rounded-xl overflow-x-auto border border-border shadow-sm mb-8 text-sm leading-relaxed">
                {children}
            </pre>
        );
    },
    ...
    h1: ({ children }: { children: React.ReactNode }) => (
        <h1 className="text-3xl md:text-4xl font-bold text-primary mb-6 mt-12 first:mt-0 pb-3 border-b-2 border-border">
            {children}
        </h1>
    ),

    h2: ({ children }: { children: React.ReactNode }) => (
        <h2 className="text-2xl md:text-3xl font-bold text-primary mb-5 mt-10 border-l-4 border-primary pl-4">
            {children}
        </h2>
    ),

    h3: ({ children }: { children: React.ReactNode }) => (
        <h3 className="text-xl md:text-2xl font-semibold text-primary mb-4 mt-8">
            {children}
        </h3>
    ),
    ...
    li: ({ children }: { children: React.ReactNode }) => (
        <li className="text-foreground leading-relaxed pl-2 relative before:content-['▸'] before:absolute before:-left-6 before:text-primary before:font-bold">
            {children}
        </li>
    ),
    ...
    a: ({ href, children }: { href?: string; children: React.ReactNode }) => {
        ...
        return (
            ...
            >
                {children}
                {isExternal && <ExternalLink className="w-3.5 h-3.5 inline-block" />}
            </a>
        );
    },
