---
title: "Distributed Modular Monolith Ecommerce Quarkus - GraphQL"
...
status: "selesai"
image: "/portofolio/distributed-modular-monolith-quarkus-grpc-graphql_ecommerce/Modular-Monolith-Ecommerce-Graphql-Grpc.png"
---
...
| Domain | Capabilities |
| :--- | :--- |
| **Auth & Users** | Secure registration, multi-factor login, stateless JWT access/refresh token lifecycle, password reset workflows, OTP email verification, and `getMe` profile GraphQL query. |
...
graph TB
    ...
    subgraph APIGateway["API Gateway — NGINX + Quarkus GraphQL Gateway"]
        direction LR
        GraphQL["GraphQL API Handler<br/>Port :5000"]:::gateway
        AuthMW["JWT Auth & Role<br/>Middleware"]:::gateway
    end

    Client -->|HTTP GraphQL| APIGateway
    ...
    GraphQL -->|"Quarkus gRPC Client"| PROD
    GraphQL -->|"Quarkus gRPC Client"| CAT
    GraphQL -->|"Quarkus gRPC Client"| BANNER
    GraphQL -->|"Quarkus gRPC Client"| SLIDER
    GraphQL -->|"Quarkus gRPC Client"| CART
    GraphQL -->|"Quarkus gRPC Client"| ORDER
    GraphQL -->|"Quarkus gRPC Client"| TXN
    GraphQL -->|"Quarkus gRPC Client"| REV
...
sequenceDiagram
    ...
    C->>GW: HTTP GraphQL Request (Query/Mutation)
    ...
    GW-->>C: HTTP GraphQL Response (JSON format)
```
...
```sh
git clone https://github.com/MamangRust/modular-monolith-ecommerce-quarkus-graphql.git
cd modular-monolith-ecommerce-quarkus-graphql
```
...
| **API Gateway Direct GraphQL Hub** | [http://localhost:5000/q/graphql](http://localhost:5000/q/graphql) |
...
├── gateway/                        # GraphQL API Gateway (GraphQL Router proxying to gRPC)
...
[📌 View on GitHub](https://github.com/MamangRust/modular-monolith-ecommerce-quarkus-graphql)
