---
title: "Distributed Modular Monolith Payment Gateway Quarkus - GraphQL"
...
status: "selesai"
image: "/portofolio/distributed-modular-monolith-quarkus-grpc-graphql_payment_gateway/Modular-Monolith-Payment-Gateway-Graphql-Grpc.png"
---

# Distributed Modular Monolith — Payment Gateway Platform (Java Quarkus GraphQL + gRPC)
...
| Domain | Capabilities |
| :--- | :--- |
| **Auth & Users** | Secure registration, multi-factor login, stateless JWT access/refresh token lifecycle, password reset workflows, OTP email verification, and `me` profile query via GraphQL Gateway. |
...
The platform implements a **Distributed Modular Monolith** architecture. Each business service is logical, decoupled, and self-contained inside its own Maven submodule, possessing its own independent gRPC boundary. A **Quarkus GraphQL API Gateway** acts as the unified edge router, transforming client GraphQL queries and mutations into fast gRPC downstream communications via Quarkus gRPC clients.
...
graph TB
    ...
    Client["Client Applications<br/>(GraphQL Clients / Web / Mobile)"]:::client

    subgraph APIGateway["API Gateway — NGINX + Quarkus GraphQL Gateway"]
        direction LR
        GRAPHQL["GraphQL API Handler<br/>Port :5000 /graphql"]:::gateway
        AuthMW["JWT Auth & Role<br/>Middleware"]:::gateway
    end

    Client -->|GraphQL POST / graphql| APIGateway
    ...
    GRAPHQL -->|"Quarkus gRPC Client"| AUTH
    GRAPHQL -->|"Quarkus gRPC Client"| USER
    GRAPHQL -->|"Quarkus gRPC Client"| ROLE
    GRAPHQL -->|"Quarkus gRPC Client"| MERCH
    GRAPHQL -->|"Quarkus gRPC Client"| CARD
    GRAPHQL -->|"Quarkus gRPC Client"| SALDO
    GRAPHQL -->|"Quarkus gRPC Client"| TOPUP
    GRAPHQL -->|"Quarkus gRPC Client"| TXN
    GRAPHQL -->|"Quarkus gRPC Client"| TRANSFER
    GRAPHQL -->|"Quarkus gRPC Client"| WITHDRAW
    ...
    GRAPHQL -->|"Quarkus Redis client"| REDIS
...
graph LR
    ...
    subgraph Gateway
        API["API Gateway<br/>Quarkus GraphQL Gateway"]:::gw
    end
...
### Synchronous Flow (GraphQL Resolvers & Cache Read-Through)

All external client API requests go through the GraphQL resolvers defined in the Quarkus API Gateway Router. The API Gateway validates the JWT/API Key, connects with the correct downstream gRPC modular server, checks the Redis Cluster cache, and fetches PostgreSQL through PgBouncer if a cache miss occurs.

```mermaid
sequenceDiagram
    autonumber
    participant C as Client
    participant GW as API Gateway<br/>(Quarkus GraphQL Gateway)
...
flowchart TB
    ...
    Client["Client Applications<br/>(HTTPS Queries/Mutations)"]:::client
...
| **API Edge Gateway** | Quarkus SmallRye GraphQL | MicroProfile GraphQL Gateway mapping queries and mutations down to gRPC. |
...
```sh
git clone https://github.com/MamangRust/modular-monolith-payment-gateway-quarkus-graphql.git
cd modular-monolith-payment-gateway-quarkus-graphql
```
...
| **API Gateway GraphQL Hub** | [http://localhost:5000/graphql](http://localhost:5000/graphql) |
| **GraphQL Playground UI** | [http://localhost:5000/q/graphql-ui](http://localhost:5000/q/graphql-ui) |
...
[📌 View on GitHub](https://github.com/MamangRust/modular-monolith-payment-gateway-quarkus-graphql)
